...
>
> **Companion docs:** [Vigil_Design_Document.md](Vigil_Design_Document.md) v1.2 (the full product spec; §5 is the screen inventory) and [CONTEXT.md](../CONTEXT.md) (the glossary). **Use glossary terms exactly** in all UI copy.

---

## 1. What Vigil is

Vigil is a clinical decision support tool for an oncology practice. It turns patient documents (scans, faxes, phone photos, PDFs) into a verified, structured **Clinical Record**, and shows the clinician standard-of-care **Treatment Options**, PBS drug information and clinical trials to consider. **The clinician decides; Vigil only informs.** This rule shapes every word and colour in the UI (§6).
...
|---|---|---|
| **Clinician** (oncologist) | Reads Patient Summaries, reviews and signs off facts (only a clinician can sign off Diagnosis, Stage, Recurrence), runs trial matches, signs off exports | Yes |
...
- **Patient context:** inside a Patient, a sticky **Patient header** shows name, DOB/age, MRN, Pseudonym (small, violet, mono), Diagnoses as chips, Open Items count, and Care Team primary.
...
The scaffold should include a **role switcher** (dev-only affordance in the top bar) to preview each Job Title's navigation.
...
| **DiagnosisBlock** | Cancer Type, histology, Stage (at dx), Disease Extent (now), BiomarkerChips, Recurrences (confirmed/suspected). |
...
### Phase 1 screens (build these first; ticket #11 onwards needs them)
...
- Overview content: DiagnosisBlocks, Treatment Course Timeline, ECOG badge, CNS status panel (only when present), key lab sparklines, **Care Team** card (role, Provider, dates, primary), Open Items.
...
- Sections in this order, each with "Last updated · SourceLink":
  ...
  2. Diagnosis: one DiagnosisBlock per Diagnosis.
  ...
  7. Medical History: Comorbidities (active/resolved), current Medications.
  8. Open Items.
- Must fit the key facts **above the fold on a 1440×900 screen**.

**9. Clinical Data Viewer**
- Sub-tabs: Labs / Imaging & Findings / Response Assessments / Biomarkers / Treatment Courses / Performance Status / Comorbidities / CNS.
...
- Reconciliation cards: "Is 'Norvasc 5mg' the same as 'Amlodipine 5mg'?", with Same / Different / Update existing / Reject. The same pattern is used for Comorbidities.
...
**13. Match Board**
- Target Diagnosis selector (preselected if only one) and "Run match".
...
- **Patients:**
  1. **Jane Citizen**, DOB 03/04/1962, MRN 1002003, Pseudonym `VG-0042`.
     - Diagnosis: NSCLC adenocarcinoma, Stage IIIA (dx 2024), Disease Extent **metastatic**.
     ...
     - Comorbidities: type 2 diabetes (active), knee OA (active), DVT 2019 (resolved).
     - ECOG 1.
  ...
  2. **Sam Example**, DOB 17/09/1975, MRN 1002017, Pseudonym `VG-0043`.
     - Diagnosis 1: breast, Stage IIA, localised, ER+/PR+/HER2−; adjuvant letrozole ongoing.
     - Diagnosis 2: melanoma 2023, no evidence of disease (this is the other-malignancy exclusion case).
