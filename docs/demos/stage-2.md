...
2. Stage 2 has shipped (`SHIPPED_STAGE = 2`): Patients, Providers and Settings → Sites appear for the Job Titles allowed to use them, with no dev toggle.
...
| 2 | Open Jane Citizen. | The Patient header (name, DOB, MRN, Pseudonym, and her treating oncologist) and the Overview's Patient Identity: Medicare + IRN, IHI, contact details, next of kin, in full. Only the Overview tab shows: the rest arrive with their stages. | #8 |
...
| 5a | On Alex Test's Overview, **Remove Patient**, reason "Added by mistake". Then paste their URL back in. | Gone from the Patient List and search, never erased: the page says "This Patient was removed by Jordan Park (synthetic) (Secretary) on <date>: Added by mistake". | #17 |
...
| 8 | As Jordan Park (Harbourside), open **Providers**. Filter Specialty = General practice. | Dr Morgan Grey and Dr Jamie Lane, the referring GPs. Search by name; filter internal/external. | #7 |
...
| 11a | Back on Jane Citizen's Overview, look at **Care Team**: Dr Alex Rivera, treating oncologist (primary), and Dr Morgan Grey, referring GP. **Add to Care Team**: Dr Taylor Quinn, Surgeon, start 01/06/2024. Then **End** Dr Quinn's membership on 30/06/2024. | Every add and end is signed off. The ended membership moves to Past. | #9 |
| 11b | Click **Dr Alex Rivera** in the Care Team. | The Provider page: their details, and the Patients they're involved with (Jane Citizen, treating oncologist, current). | #9 |
...
| 13 | **Add Site**: another hospital clinic, *Example Private Hospital clinic*, with an address and a location → Add Site. Then **Make primary**, and try to delete the old primary. | The primary moves; the primary Site can't be deleted. Every change is in the audit trail. | #25 |
