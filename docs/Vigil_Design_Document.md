...
- `care_team_member(patient_id) WHERE is_primary AND deleted_at IS NULL`: at most one primary Care Team member per Patient.
...
| `care_team_member` | A Provider's role in one Patient's care. Ending one is setting `end_date` (it then shows as past); the current primary member heads the Patient header. Every add, change or end is an `edit` Verification on `subject_table = 'care_team_member'`. | `patient_id`, `provider_id`, `role`, `is_primary` (at most one live primary per Patient; choosing another moves it, signed off too), `start_date`, `end_date` (null = current), `notes` |
...
| `job` | One Job on the durable queue (§3). **Payload holds IDs only.** | `kind` (FK → `job_kind.key`), `status`, `practice_id` (nullable: null = system-wide, e.g. a Refresh), `payload` (JSONB, IDs only), `priority`, `attempts`, `max_attempts`, `run_after`, `locked_at`, `locked_by` (worker ID), `last_error` (IDs only), `pipeline_run_id` (nullable), `finished_at`. The queue refuses to enqueue or run a Job whose Job Kind's module isn't active for its Practice. **Claims and leases:** a worker claims a Job (`locked_by`, `locked_at`) and renews its claim in the background while it runs it, a few times per lease (`DEFAULT_LEASE`, 15 minutes), however long a step takes (#31). A Job whose claim goes unrenewed for the lease belonged to a worker that died, and is claimed again. A renewal by a worker that no longer holds the Job changes nothing, and a worker renews once more before recording a step, success or failure: if another worker has taken the Job over, it stops and records nothing. |
...
> **Claude Code note:** Enforce this in the service layer, not in the UI alone. The single permission check is `core/permissions.py`: `may(job_title, permission)` for the administrative and sign-off rows, and `VerificationRights.can_verify(job_title, fact_kind)` for fact kinds (Core rows there; each Specialty Module contributes its rows, combined by the module registry). Services call it with the signed-in User's Job Title and refuse with 403. The UI shows facts a User can't verify as read-only with a "needs clinician" badge. Clinician-only actions require re-entering the password or TOTP code within the last 5 minutes, recorded as `verification.reauthenticated = true`.
